"""Top 10 Beta 616 — experimental mature-sample lead-lag challenger.

The model retains the validated residual lead-lag network, but partially
adjusts the alpha residuals for heterogeneous ALGO betas once enough history
exists.  The beta adjustment is shrunk heavily toward the stable beta-one
prior and ramped in by sample size.  Inputs are sanitized without external
data or file access.
"""
...
def _sanitize_prices(values):
    ...
    filled = np.where(
        columns[None, :] < first_valid[:, None], first_values[:, None], filled
    )
...
def getMyPosition(prcSoFar):
    ...
    n_returns = log_returns.shape[1]
    if n_instruments != 51 or n_returns < MIN_HISTORY:
        _last_day_count = n_days
        _last_position = positions
        return positions.copy()

    factor = log_returns[0] - log_returns[0].mean()
    beta_denominator = max(float(factor @ factor), 1e-12)
    raw_betas = (
        (log_returns[1:] - log_returns[1:].mean(axis=1, keepdims=True)) @ factor
    ) / beta_denominator
    maturity = float(
        np.clip(
            (n_returns - BETA_RAMP_START) / (BETA_RAMP_END - BETA_RAMP_START),
            0.0,
            1.0,
        )
    )
    alpha_betas = 1.0 + maturity * ALPHA_BETA_SHRINKAGE * (raw_betas - 1.0)

    residuals = log_returns[1:] - alpha_betas[:, None] * log_returns[0]
